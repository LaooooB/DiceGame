...
public sealed partial class Simulation
{
    ...
    private ShotStats EnhanceContentStats(DieState die, ShotStats q)
    {
        ...
        double auraDamage = 0, auraEcho = 0, auraPierce = 0, auraSplit = 0;
    ...
    {
        var q = Stats(die);
        return q.Count * (q.Trait("echoChance") > 0 ? 1 + Math.Clamp((int)q.Trait("echoCopies", 1), 1, 2) : 1);
    }
    ...
    public int RecycleValue(DieState die) => C.Levels[die.Pips-1].Recycle + ContentRecycleBonus(die);
    ...
    private void QueueContentVolley(DieState die, int slot, double angle, double multiplier, bool surge)
    {
        ...
        var stats = Stats(die); stats.Damage *= multiplier; stats.Volley *= multiplier;
        ...
        for (int k = 0; k < stats.Count; k++)
        {
            ...
            S.PendingShots.Add(new PendingShot { Due = S.Time + .075 + k * .056 + slot % C.Board.Columns * .019 + copy * .15, Angle = angle, Snapshot = snapshot });
        }
