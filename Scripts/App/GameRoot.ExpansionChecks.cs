...
public partial class GameRoot
{
    ...
    private async Task<int> CheckExpansionControls(string path)
    {
        ...
        var button=Walk(_ui).OfType<Button>().Single(b=>b.Text=="裁定");
        ...
        button=Walk(_ui).OfType<Button>().Single(b=>b.Text=="裁定");Check("spent adjudication is disabled",button.Disabled);
        ...
        await Frames();App.OpenDie(0);await Frames();
        Check("six-pip reincarnation has a native details page",App.Scene=="die");
        var rebirth=Walk(_ui).OfType<Button>().Single(b=>b.Text=="主动转世");rebirth.EmitSignal(Button.SignalName.Pressed);await Frames();
        var confirmation=_ui.GetNode<Control>("Overlay/Center/ConfirmPanel");
        ...
        screens++;confirmation.GetNode<Button>("Margin/Content/Actions/Confirm").EmitSignal(Button.SignalName.Pressed);await Frames();
