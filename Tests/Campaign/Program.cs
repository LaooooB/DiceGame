...
internal static class Program
{
    ...
    public static int Main()
    {
        ...
        { var a=App();Eq(a.Scene,"town");Eq(a.Deck.Count,6);Check(a.Campaign!.UnlockedDice.SetEquals(a.Deck.Concat(Data.Dice.Where(d=>d.AvailableFromStart).Select(d=>d.Id))));Eq(Catalog.Regions.Keys.Count(id=>Progress.IsRegionOpen(a.Campaign,id)),1); });
        ...
        { var a=App();Check(a.StartExpedition(7));Eq(a.Sim!.State.Board.Length,16);Eq(a.Sim.State.Expedition!.LeadDice,a.Deck[0]);Check(a.Sim.State.Deck.All(a.Campaign!.UnlockedDice.Contains)); });
        ...
        {var store=new MemoryStorage{Value=SaveCodec.Encode(new SaveEnvelope{Version=1,Deck=Data.DefaultDeck.ToList()})};var a=App(store);Eq(a.Deck.Count,6);Check(a.Campaign!.UnlockedDice.SetEquals(Data.DefaultDeck.Concat(Data.Dice.Where(d=>d.AvailableFromStart).Select(d=>d.Id))));a.Save();Eq(SaveCodec.Decode(Data,store.Value!).Version,4);});
        Test("legacy active run migration: not replaced or restarted",()=>
        {var old=new Simulation(GameData.FromDirectory(Path.Combine(AppContext.BaseDirectory,"LegacyBalanceData")),Data.DefaultDeck,710);old.Fire(-1.6);for(int i=0;i<30;i++)old.Step(1.0/120);var before=old.ExportSave();var store=new MemoryStorage{Value=SaveCodec.Encode(new SaveEnvelope{Version=1,Deck=before.Deck,Run=before})};var a=App(store);Check(a.ResumeData!.Expedition is {LegacyRules:true,Endless:true});Eq(a.ResumeData.Wave,before.Wave);Eq(a.ResumeData.Rng,before.Rng);Eq(Encode(a.ResumeData.Board.Take(8)),Encode(before.Board));Eq(a.ResumeData.Board.Length,16);a.Resume();Check(a.Sim!.State.Expedition is {LegacyRules:true});Check(a.Sim.State.Time==before.Time);});
        ...
        {var s=Progress.NewState();s.UnlockedDice.UnionWith(Data.Dice.Select(d=>d.Id));var e=Progress.PrepareRun(s,"region_01",Data.DefaultDeck,1);var sim=new Simulation(Data,Data.DefaultDeck,1,e);ClearRegion(sim);Progress.Settle(s,sim.State);Eq(s.Gears,1);});
