...
internal static partial class Program
{
    ...
    static void Mythic()
    {
        Test("fate A: two different numbers bless matching real pip counts",()=>{
            var s=Sim("fate");Die(s,"fate",6,"A","D",23);Enemy(s);Clock(s,.05);var x=s.State.Expansion;Eq(x.FateNumbers.Count,2);Eq(x.FateNumbers.Distinct().Count(),2);
        ...
        Test("fate B: a twelve-second strong fate window uses its separate PRNG",()=>{
            var s=Sim("fate");Die(s,"fate",6,"B","D",23);Enemy(s);Clock(s,.05);uint law=s.State.Expansion.FateRng,combat=s.Random.State;
        ...
        Test("fate C: a matching rule owner grants a timed global blessing",()=>{
            var s=Sim("fate");Die(s,"fate",6,"A","C",23);Enemy(s);
        ...
        Test("fate D: lowest-instance blessing is captured without changing real pips",()=>{
            var s=Sim("fate");Die(s,"fate",6,"B","D",23);var low=Die(s,"pulse",1,slot:0);Die(s,"blast",2,slot:1);Enemy(s);Clock(s,.05);Eq(low.Pips,1);Check(s.State.Expansion.FateMinimumIds.SequenceEqual(new[]{low.Id}));s.State.Expansion.FateNumbers=[6];Near(s.Stats(low).Volley,Base(low)*1.75);
        ...
        Test("order A: next-result preview is pure and matches the committed merge",()=>{
            var s=Sim("order");Die(s,"order",3,"A","",23);string before=J(s.ExportSave());string expected=s.OrderPreview().Single();Eq(J(s.ExportSave()),before);Eq(MergeOne(s),expected);
        });
        Test("order B: bag boundaries never repeat the previous last type",()=>{
            var s=Sim("order");Die(s,"order",3,"B","",23);string last="";for(int bag=0;bag<10;bag++){var draws=Enumerable.Range(0,6).Select(_=>MergeOne(s)).ToArray();Eq(draws.Distinct().Count(),6);Check(draws[0]!=last);last=draws[^1];}
        });
        Test("order C: three-item prediction crosses bags without consuming either random stream",()=>{
            var s=Sim("order");Die(s,"order",6,"A","C",23);for(int i=0;i<4;i++)MergeOne(s);string before=J(s.ExportSave());var prediction=s.OrderPreview().ToArray();Eq(prediction.Length,3);Eq(J(s.ExportSave()),before);var results=Enumerable.Range(0,3).Select(_=>MergeOne(s)).ToArray();Check(prediction.SequenceEqual(results));
        });
        Test("order D: one rotation per stage preserves all six cards",()=>{
            var s=Sim("order");Die(s,"order",6,"A","D",23);string first=s.OrderPreview()[0];Check(s.SkipOrder().Ok);Check(!s.SkipOrder().Ok);var results=Enumerable.Range(0,6).Select(_=>MergeOne(s)).ToArray();Eq(results.Distinct().Count(),6);Eq(results[^1],first);s.State.Wave=6;Check(s.CanSkipOrder);Check(s.SkipOrder().Ok);
        });
    }
}
