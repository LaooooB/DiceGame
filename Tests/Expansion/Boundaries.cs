...
internal static partial class Program
{
    static void Boundaries()
    {
        ...
        Test("mythic copies: fate and void global laws do not scale with instance count",()=>{
            var fate=Sim("fate");Die(fate,"fate",6,"B","D",15);var ally=Die(fate,"pulse",1,slot:0);fate.State.Expansion.FateNumbers=[1];double one=fate.Stats(ally).Volley;Die(fate,"fate",6,"B","D",14);Near(fate.Stats(ally).Volley,one);
            var empty=Sim("void");Die(empty,"void",6,"A","C",15);var friend=Die(empty,"pulse",1,slot:0);one=empty.Stats(friend).Volley;Die(empty,"void",6,"A","C",14);Near(empty.Stats(friend).Volley,one);Eq(empty.Stats(friend).Pierces,1);
        });
        ...
        Test("busy merge: validation precedes consumption of the order bag and materials",()=>{
            var s=Sim("order");Die(s,"order",6,"A","D",15);var d=Die(s,"pulse",1,slot:0);Die(s,"pulse",1,slot:1);var snap=new ShotSnapshot{Type="pulse",SourceDieId=d.Id,Stats=s.Stats(d)};
        ...
        Test("echo: one normal release creates bounded snapshots without repeated beat counters",()=>{
            ...
            var data=new GameData(J(Data.Game),J(defs),J(Data.Upgrades),J(Data.Skills.Values));var s=new Simulation(data,new[]{"rhythm","pulse","blast","arc","frost","split"},15);Array.Clear(s.State.Board);s.State.Enemies.Clear();var d=Die(s,"rhythm",6,"B","D");d.NormalAttacks=4;Check(s.Fire(-1.5).Ok);Eq(d.NormalAttacks,5L);Eq(d.BeatRemainders,2);Eq(s.State.PendingShots.Count,18);Eq(s.State.PendingShots.Count(p=>p.Snapshot.Stats.Trait("rootFirst")>0),1);
        });
        ...
        Test("order preservation: removing and returning the die cannot reroll the unfinished bag",()=>{
            var s=Sim("order");var d=Die(s,"order",6,"A","C",15);MergeOne(s);string before=string.Join(",",s.State.Expansion.OrderBag);s.Recycle(15);Die(s,"order",6,"A","C",15);Eq(string.Join(",",s.State.Expansion.OrderBag),before);
        });
        Test("order last card: adjudication cannot discard the only remaining result",()=>{
            var s=Sim("order");Die(s,"order",6,"A","D",15);for(int i=0;i<5;i++)MergeOne(s);Eq(s.State.Expansion.OrderBag.Count,1);string before=J(s.ExportSave());Check(!s.SkipOrder().Ok);Eq(J(s.ExportSave()),before);
        });
        Test("transaction: order skip is stored before publishing and disk failure consumes nothing",()=>{
            var store=new Store();var app=App(store,"order");Die(app.Sim!,"order",6,"A","D",15);app.Save();string before=J(app.Sim!.ExportSave());store.Fail=true;Check(!app.UseOrderSkip());Eq(J(app.Sim.ExportSave()),before);store.Fail=false;Check(app.UseOrderSkip());var saved=SaveCodec.Decode(Data,store.Value!);Check(saved.Run!.Expansion.OrderSkipStage>=0);Check(!app.UseOrderSkip());
        });
        ...
        Test("deterministic mixed save: fields, portals, swarm and order replay identically",()=>{
            var s=Sim("blackhole","rift","swarm","spree","barrier","order");for(int i=0;i<16;i++)Die(s,s.State.Deck[i%6],6,i/4%2==0?"A":"B",i%4==3?"D":"C",i);for(int i=0;i<6;i++)Enemy(s,70+i*55,210,1e10);
        ...
        Test("stress: all five mythic deck variants remain bounded under twenty-four active dice",()=>{
            ...
            {
                var s=Sim(mythic,"blackhole","swarm","spree","rift","reactor");for(int i=0;i<16;i++)Die(s,s.State.Deck[i%6],6,i/4%2==0?"A":"B",i/8==0?"C":"D",i);
